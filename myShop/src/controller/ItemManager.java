...
public class ItemManager {
				...
				if (catIdx >= 0 && catIdx < category.size()) {
					...
					if (selIdx == -1) {
						...
						if (tempItem != null) {
							...
							System.out.println("[아이템 번호를 확인하세요]");
					} else
						System.out.println("[옳은 메뉴를 선택하세요]");
				} else
		...
		if (CartIsEmty(logId)) {
			...
			for (int i = 0; i < cart.size(); i++) {
				if (cart.get(i).getUserId().equals(logId)) {
					System.out.printf("%d) [%s] %d원 (%s) \n", i, cart.get(i).getItem().getItemName(),
							cart.get(i).getItem().getPrice(), cart.get(i).getItem().getCategory());
				}
			}
		...
		if (!CartIsEmty(logId)) {
			...
			for (int i = 0; i < itemCnt; i++) {
				...
				if (check) {
					...
					// 출력
					System.out.printf("[%s]\t%d원(%d원 * %d개) (%s)\n", tempItem[i].getItemName(),
							(tempItem[i].getPrice() * tempCnt), tempItem[i].getPrice(), tempCnt,
							tempItem[i].getCategory());
				...
				if (catIdx == -1) {
					...
					if (getItem(category.get(catIdx), inName) == null) { // 중복아이템 이름
						...
						if (inPrice >= 0) {
							...
							System.out.println("올바른 가격 입력필요");
					}
				}
			} else
	...
	public void printAllCart() {
		...
		if (!cart.isEmpty()) {
			for (Cart c : cart) {
				System.out.printf("[id: %s] [cat: %s][item: %s, %d원]\n", c.getUserId(), c.getItem().getCategory(),
						c.getItem().getItemName(), c.getItem().getPrice());
			}
		} else
	...
	public void printPayed() {
		...
		for (int i = 0; i < payed.size(); i++) {
			...
			// 같은 아이템이 있으면 참고하지 않음
			boolean check = true;
			if (tempPoint.contains(payed.get(i).getItem()))
				check = false;
			...
			if (check) {
				...
				// 출력
				System.out.printf("[%s]\t%d원(%d원 * %d개) (%s)\n", tmpItem.getItemName(), (tmpItem.getPrice() * itemCnt),
						tmpItem.getPrice(), itemCnt, tmpItem.getCategory());
				tempPoint.add(payed.get(i).getItem()); // 중복체크를 위해 넣어준다
				sale += (tmpItem.getPrice() * itemCnt); // 총 판매액에 더해주고
			}
		}
		if (payed.isEmpty())
			System.out.println("[결제된 내역이 없습니다]");
		else
